...
class ERC20 {
  ...
  balanceOfNum(account: string, blockNum: number): Promise<BigNumber> {
    return this.contract.balanceOf(account, {blockTag: blockNum});
  }
  ...
  async displayedTotalSupply(): Promise<string> {
    ...
    return Number(formatUnits(supply, this.decimal)).toFixed(3);
  }}
